#pragma once

/**
 * ----------------------------------------
 * |        Generated By CheatGear        |
 * ----------------------------------------
 * | Game:    TFDCG                       |
 * | Version: 1                           |
 * | Date:    08/22/2024                  |
 * ----------------------------------------
 */

#include <cstdint>
#include <vector>
#include <string>
#include "BasicTypes_TMap.h"
#include "M1_FM1SeasonReinforce.h"
#include "M1_FM1AbilityId.h"
#include "M1_UM1CharacterComponent.h"
#include "M1_ENUMS.h"

// --------------------------------------------------
// # Forwards
// --------------------------------------------------
namespace CG::M1 { class AM1Player; };
namespace CG::M1 { class UM1AbilityComponent; };
namespace CG::CoreUObject { class UClass; };

#ifdef _MSC_VER
    #pragma pack(push, 0x01)
#endif

namespace CG::M1
{
    /**
     * Class /Script/M1.M1SeasonReinforceComponent
     * Size -> 0x0060 (FullSize[0x0130] - InheritedSize[0x00D0])
     */
    class UM1SeasonReinforceComponent : public M1::UM1CharacterComponent
    {
    public:
        BasicTypes::TMap<M1::FM1SeasonReinforce, M1::FM1AbilityId>   AbilityIdMap;                                            //  Transient, NativeAccessSpecifierPrivate
        M1::AM1Player*                                               Player_Owner;                                            //  ZeroConstructor, Transient, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPrivate
        M1::UM1AbilityComponent*                                     Ability_Component;                                       //  ExportObject, ZeroConstructor, Transient, InstancedReference, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPrivate

    public:
        void OnChangedSeasonReinforceSlot(M1::EM1SeasonReinforceSlotChangeType InChangeType);
        void OnChangedSeasonInfo();
        static inline CoreUObject::UClass* StaticClass()
        {
            static CoreUObject::UClass* ptr = nullptr;
            if (!ptr)
                ptr = CoreUObject::UObject::FindClass("Class /Script/M1.M1SeasonReinforceComponent");
            return ptr;
        }
    };
}

#ifdef _MSC_VER
    #pragma pack(pop)
#endif
